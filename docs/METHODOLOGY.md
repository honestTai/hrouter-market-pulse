...
Yahoo Finance provides history and quote metadata; Tencent Finance provides a comparison snapshot. Both providers are queried for the same normalized symbol. Agreement checks currency, price difference, quote timestamps and freshness. Price agreement alone is insufficient. Current regular-session signals require both usable data and the final quote-level `signalEligible` gate. Default freshness permits public-feed delay; it does not certify real-time delivery.
